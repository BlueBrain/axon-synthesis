...
def segment_region_ids(row, brain_regions):
    ...
    if transitions.any():
        ...
        if transition_indices[0][-1] != len(regions) - 1:
            transition_indices = (np.append(transition_indices[0], len(regions) - 1),)
    else:
...
def compute_clusters(self, axon, axon_id, group_name, group, _, __):
    ...
    segment_sub_edges = edges_to_cut["sub_segments"].apply(lambda x: pd.Series(x.tolist())).stack()
    ...
    for i in intermediate_target_nodes.itertuples():
        nodes.loc[
            (nodes["section_id"] == i.section_id) & (nodes["sub_segment_num"] >= i.sub_segment_num),
            "sub_segment_num",
        ] += 1
